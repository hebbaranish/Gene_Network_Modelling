...
for i in range(dict[network]):
    for j in range(i+1, dict[network]):
        for p in range(j+1, dict[network]):
            for run_num in range(1,4):
                print("processing {}{}{}".format(i+1, j+1,p+1))
                # BOOLEAN PART
                boolean_jsd.write("{}{}{} ".format(i+1, j+1,p+1))
                dict_list = []
                for k in range(0,dict[network]+1):
                    list_k = []
        
                    dict_k = boolean_probability_counter(boolean_dir,"{}{}{}_{}".format(i+1, j+1,p+1, k), num_nodes, num_const,run_num)
