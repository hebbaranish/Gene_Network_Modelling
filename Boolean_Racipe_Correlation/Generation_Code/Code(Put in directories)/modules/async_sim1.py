...
def simulate(begin,cur_run,random_seed,max_sim,link_matrix,n,params,filename,counter,counter1,id_counter):
    ...
    while sim_num < max_sim:
        ...
        for timeiter in range(params['maxtime']):
            ...
            #for i in range(1,n):
            #    if(activation[i]>=0):
            #        activation[i]=0.5+ abs(activation[i])/(2*abs(activation[i] +1) 
            #    else:
            #        activation[i]=0.5- abs(activation[i])/(2*abs(activation[i] +1)
            #    activation[i]-=(1+curstate[i])/2 *  
            #for b in range(1,n):
            ...
            if stable_checker:
                ...
                while(qq==0):
                    ...
                    if ((activation[randnode])>0 and (curstate[randnode]) <0  ):
                        curstate[randnode]=1
                        qq=1
                    elif ((activation[randnode])< 0 and (curstate[randnode]) > 0 ):
                        curstate[randnode]=-1
                        qq=1
                #for i in range(1,n):
                #    if(activation[i]<0 ):
                #        curstate[i]=max(curstate[i]+0.1*activation[i],-1)
                #    elif(activation[i]>0 ):
                #        curstate[i]=min(curstate[i]+0.1*activation[i],1)
                # print(link_matrix)
                # print(activation)
                # print(randnode)
                #qq=0
