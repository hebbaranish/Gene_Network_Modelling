...
for i in range(num_edges):
    for j in range(i+1, num_edges):
            ...
            # print(tempstr)
            # exit(0)
            p = open("{}/{}{}_{}.topo".format(dirname, i+1, j+1, 0), "w")
            p.write(topo.format(*temp))
            p.close()
            q = open("{}/{}{}_{}.ids".format(dirname, i+1, j+1, 0), 'w')
            q.write(idsfile)
            q.close()
            for k in range(1,num_edges + 1):
                ...
                for l in tempstr:
                    if l != tempstr[k]:
                        finstr += l + "\n"
                # print(finstr)
                p = open("{}/{}{}_{}.topo".format(dirname, i+1, j+1,  k), 'w')
                p.write(finstr)
                p.close()
                q = open("{}/{}{}_{}.ids".format(dirname, i+1, j+1,  k), 'w')
                q.write(idsfile)
                q.close()
                # exit(0)
