...
void check_inputfile (int argc, char **argv, struct topo *topoinfo, struct opts *simu_opts, struct rlt *tmprlt)
{
  ...
  if (argc == 1) {
    ...
    printf("-maxthreads    : Maximum threads being used. Compulsory input.\n");
    printf("-curthread     : Current thread (for time purposes). Compulsory input.\n");
  ...
  if (argc >= 3){
    ...
    for (i = 2; i < argc; i = i + 2){
      ...
      if (strcmp(argv[i], "-dist") == 0){
        ...
        maxthreads = atof(argv[i+1]);
      }
      else if (strcmp(argv[i], "-curthread") == 0){
        curthread = atof(argv[i+1]);
      }
...
void run_RACIPE(struct opts *simu_opts, struct topo *topoinfo, struct rlt *tmprlt)
{
  ...
  begin = clock();
  int perc_meow = 100; //number of percentages to be displayed
  ...
  for (i = 1; i <= simu_opts->num_paras; i++){
	if (((i-1)%(simu_opts->num_paras/perc_meow) == 0) && ((curthread)*(simu_opts->num_paras/maxthreads) <= i && ((curthread+1)*(simu_opts->num_paras/maxthreads) > i))){
		//printf("%d ", i);
		printf("\r%d%% %.2fs",( (i-1)/(simu_opts->num_paras/perc_meow) )*(100/perc_meow),omp_get_wtime()-timecount);
		fflush(stdout);
		//printf("%d%% ", ((i-1)/(simu_opts->num_paras/perc_meow))*(100/perc_meow));
		// printf("%ds \n", ((clock() - begin)/CLOCKS_PER_SEC/1));
		//printf("%.2fs\n", (omp_get_wtime() - timecount));
	}
