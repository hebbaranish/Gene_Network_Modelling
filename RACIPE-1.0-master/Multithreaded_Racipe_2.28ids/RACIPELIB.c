...
void check_inputfile (int argc, char **argv, struct topo *topoinfo, struct opts *simu_opts, struct rlt *tmprlt)
{
  ...
  if (strcmp(fileextension, "topo") == 0) {
    ...
    Model_generate1   (argv[1],argv[2], topoinfo, simu_opts, tmprlt);
  }
...
// Generate the models, parameter ranges for randomization and the configure file. (if cfg file)
void Model_generate (char *inputfile, struct topo *topoinfo, struct opts *simu_opts, struct rlt *tmprlt)
{
  ...
  if (simu_opts->exts == 0){
	...
	tmprlt->stbstate =  (int *)    calloc(simu_opts->num_stability,                           sizeof(int));
    tmprlt->Nstb      = 0;
...
// Generate the models, parameter ranges for randomization and the configure file. (if topo file)
void Model_generate1 (char *inputfile,char *idsfile, struct topo *topoinfo, struct opts *simu_opts, struct rlt *tmprlt)
{
  int i = 0;
  char KDIDname[100] = "";
  char OEIDname[100] = "";
  ...
  if (simu_opts->exts == 0){
    check_topo1(fin, fout,ids, topoinfo); // read in the topo file
    fclose(fin);
	fclose(ids);
    // initial struct rlt *tmprlt
    tmprlt->Nstb      = 0;
...
// Read in the topology information
void check_topo1(FILE *f_in, FILE *f_out,FILE *ids, struct topo *topoinfo)
{
  int  tmpnumG  = 0;
  int  tmpnumR  = 0;
  int  RT       = 0;  //regulation type
  ...
  fscanf(ids, "%*[^\n]\n", NULL); //skip first line of ids file
  for( int i=0;i<count;i++){
	   fscanf(ids, "%s %d\n", G3, &idnum );
	   topoinfo->Gname[i] = (char*)malloc(sizeof(G3));
       strcpy(topoinfo->Gname[i], G3);
  }
...
for(int u=0;u<threads;u++){
	...
	threadtmprlt[u]->stbstate =  (int *)    calloc(simu_opts->num_stability,                           sizeof(int));
...
void save_model_solns(struct opts *simu_opts, struct topo *topoinfo, struct rlt *tmprlt, int num)
{
  ...
  fprintf(f_s[n], "%d\t%d\t%d", num, cnt,tmprlt->stbstate[0]);
    ...
    for (h = 0; h < cnt; h++){
        ...
        if(h!=cnt-1){
			fprintf(f_s[n],"\n");
			fprintf(f_s[n], "%d\t%d\t%d",num, cnt,tmprlt->stbstate[h+1]);
		}		
    }
  fprintf(f_s[cnt-1], "\n");
  fprintf(f_s[n],"\n");
...
// detect the stable states from all the solutions from different RIVs
{
	...
	int countstate=0;
	for (int gg=1; gg<=simu_opts->num_ode; gg++){
		h = 1;
		sumpow = 0.0;
		while (h <= topoinfo->numG) {
			sumpow = sumpow + pow((tmprlt->y_store[topoinfo->numG*(gg-1) + h - 1] - tmprlt->soln[(cnt-1)*topoinfo->numG + h - 1]), 2);
			h++;
		}
		...
		tmprlt->stbstate[cnt-1]=countstate;	
